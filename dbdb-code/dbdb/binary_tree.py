...
'''
binary_tree.py 定义了逻辑接口下的具体的二叉树算法。
 1、BinaryTree 实现二叉树及其基本操作。值得注意的是我们实现的是一个数据不可变的二叉树，
    每次数据更新都会返回一颗新树,新树的大部分数据由于同旧树一致所以直接与救赎共享那部分数据。

 2、BinaryNode 实现二叉树中的节点

 3、BinaryNodeRef 是ValueRef 的子类，实现对二叉树的引用。
'''
